...
vboxmanage guestcontrol "UQM_crossbuilder_001" execute "/usr/bin/sudo" --username "user" --password "live" --verbose $WAITSTDOUT $WAITSTDERR "/bin/mkdir" "/vbox_share"
if [ $? -ne 0 ]; then
...
vboxmanage guestcontrol "UQM_crossbuilder_001" execute "/usr/bin/sudo" --username "user" --password "live" --verbose $WAITSTDOUT $WAITSTDERR -- "/bin/touch" "/vbox_share/touched_file"
if [ $? -ne 0 ]; then
